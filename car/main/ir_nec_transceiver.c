...
//void car_hit(uint8_t); //stored in main.c hopefully no error
...
{
	//end of section that may need to be deleted !note
...
void init_ir_transceiver(){
	...
	QueueHandle_t receive_queue = xQueueCreate(1, sizeof(rmt_rx_done_event_data_t));
		...
		//potentially delete this section !note
		ESP_ERROR_CHECK(rmt_receive(rx_channel, raw_symbols, sizeof(raw_symbols), &receive_config));
		if (xQueueReceive(receive_queue, &rx_data, pdMS_TO_TICKS(1000)) == pdPASS) {
			// parse the receive symbols and print the result
			example_parse_nec_frame(rx_data.received_symbols, rx_data.num_symbols);
		}
		//end of section to potentially delete !note
}
